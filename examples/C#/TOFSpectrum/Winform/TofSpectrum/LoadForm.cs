...
namespace Oscilloscope
{
    public partial class LoadForm : Form
    {
        ...
        private void btn_connect_Click(object sender, EventArgs e)
        {
            ...
            JObject tof_spectrum_obj = null;
            string tof_spectrum_name = "";
            ...
            else
            {
                ...
                try
                {
                    ...
                    for(int i = 0; i < mmc_components_obj.Count; i++)
                    {
                        ...
                        tof_spectrum_name = (string)mmc_components_obj[i]["Name"];
                        if (component_type == "TofSpectrum")
                        {
                            tof_spectrum_obj = (JObject)mmc_components_obj[i];
                            found = true;
                            break;
                        }
                    }
                    if (!found)
                    {
                        MessageBox.Show("Loaded configuration from json file does not contain TOF spectrum", "Error");
                        return;
                    }
                }
                catch (Exception ex)
            ...
            if (cmb_connection_type.Text == "USB")
            {
                ...
                if (int.TryParse(board_sn, out serial_number))
                {
                    ...
                    if (res == 0)
                    {
                        ...
                        TOFSpectrumForm oscilloscope_form = new TOFSpectrumForm(sdk, board_name, tof_spectrum_name, txt_json_file.Text, tof_spectrum_obj);
                        this.Hide();
