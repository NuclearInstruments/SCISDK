            ...
            else
            {
                ...
                try
                {
                    ...
                    for(int i = 0; i < mmc_components_obj.Count; i++)
                    {
                        ...
                        digitizer_name = (string)mmc_components_obj[i]["Name"];
                        if (component_type == "WaveDump")
                        {
                            digitizer_obj = (JObject)mmc_components_obj[i];
                            found = true;
                            break;
                        }
                    }
                    if (!found)
                    {
                        MessageBox.Show("Loaded configuration from json file does not contain an oscilloscope dual", "Error");
                        return;
                    }
                }
                catch (Exception ex)
            ...
            if (cmb_connection_type.Text == "USB")
            {
                ...
                if (int.TryParse(board_sn, out serial_number))
                {
                    ...
                    if (res == 0)
                    {
                        ...
                        DigitizerForm oscilloscope_form = new DigitizerForm(sdk, board_name, digitizer_name, txt_json_file.Text, digitizer_obj);
                        this.Hide();
