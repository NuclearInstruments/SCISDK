...
SciSDK_Oscilloscope::SciSDK_Oscilloscope(SciSDK_HAL *hal, json j, string path) : SciSDK_Node(hal, j, path)
{
...
NI_RESULT SciSDK_Oscilloscope::ISetParamI32(string name, int32_t value) {
...
NI_RESULT SciSDK_Oscilloscope::ISetParamString(string name, string value) {
...
NI_RESULT SciSDK_Oscilloscope::IGetParamU32(string name, uint32_t *value) {
...
NI_RESULT SciSDK_Oscilloscope::IGetParamI32(string name, int32_t *value) {
...
NI_RESULT SciSDK_Oscilloscope::IGetParamString(string name, string *value) {
	if (name == "trigger_mode") {
...
NI_RESULT SciSDK_Oscilloscope::AllocateBuffer(T_BUFFER_TYPE bt, void **buffer) {
...
NI_RESULT SciSDK_Oscilloscope::FreeBuffer(T_BUFFER_TYPE bt, void **buffer) {
	if (bt == T_BUFFER_TYPE_DECODED) {
		if (*buffer == NULL) {
...
NI_RESULT SciSDK_Oscilloscope::ReadData(void *buffer) {
	uint32_t dv;
...
NI_RESULT SciSDK_Oscilloscope::ConfigureOscilloscope() {
	int ret = 0;
...
NI_RESULT SciSDK_Oscilloscope::CmdResetReadValidFlag() {
	int ret = 0;
	ret |= _hal->WriteReg(0, address.cmd_arm);
...
NI_RESULT SciSDK_Oscilloscope::ExecuteCommand(string cmd, string param) {
	if (cmd == "arm") {
		return CmdArm();
...
NI_RESULT SciSDK_Oscilloscope::CheckOscilloscopeStatus(bool *ready, bool *armed, bool *running) {
	uint32_t status;
...
NI_RESULT SciSDK_Oscilloscope::ReadStatus(void *buffer) {
