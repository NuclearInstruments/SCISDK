...
public class SciSDK {
    ...
    // allocate spectrum decoded buffer
    public int AllocateBuffer(String path, AtomicReference buffer) {
        // SPECTRUM DECODED BUFFER
        if (buffer.get().getClass() == SpectrumDecodedBuffer.class) {
        ...
        else if (buffer.get().getClass() == OscilloscopeDecodedBuffer.class) {
    ...
    // read decoded data from spectrum
    public int ReadData(String path, AtomicReference buffer) {
        // SPECTRUM DECODED BUFFER
        if (buffer.get().getClass() == SpectrumDecodedBuffer.class) {
